...
```
> マイグレーションは **D1 バインディング名 `DB`** を対象に適用する（`wrangler d1 migrations apply DB`）。
> バインディング名で指定するのは、「Deploy to Cloudflare」ボタンで配布したとき各利用者の D1 が別名で自動生成されても同じスクリプトで動くようにするため（Cloudflare 推奨）。
> `migrations/` の連番（`0001` 〜）を順に適用する。`d1_migrations` テーブルで適用済みが管理され、未適用分のみ当たる。**既存のマイグレーションは編集せず、変更は必ず新しい番号で追加すること**（配布済み環境との不整合を防ぐ）。
> ⚠️ **`npm run deploy` は内部で `db:migrate:remote`（本番 D1 への適用）を先に実行**してから `wrangler deploy` する。本番 D1 に触れるため、CLAUDE.md の本番操作ルールに従うこと。
...
## 6. デプロイ
```bash
npm run deploy   # = db:migrate:remote（本番 D1 へマイグレーション）→ wrangler deploy
```
出力される URL（例 `https://discord-event-bot.<account>.workers.dev`）を控える。
> 「Deploy to Cloudflare」ボタンで配布した利用者環境では、この `deploy` スクリプトを Cloudflare が自動検出し、初回デプロイ・以降の更新（本番ブランチへの push ごとの Workers Builds）でマイグレーションが自動適用される。利用者は CLI 不要。
