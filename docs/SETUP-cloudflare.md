...
> 本番 `.env`（Vercel版）が同居しているため、**`.dev.vars` が無いと `.env` の本番
> `DISCORD_BOT_TOKEN` / `DISCORD_CHANNEL_ID` が使われ、ローカル検証が本番チャンネルへ投稿してしまう**。
> 必ず `.dev.vars` を作成し、**テスト用アプリ＋ダミーサーバーの値のみ**を入れること（本番値は厳禁）。
> `.dev.vars` は `.env` より優先されるため、これがあれば事故を防げる。
