...
```bash
# ローカル UI 起動（Phase 3 統合後）
node design-system/serve.mjs --ui    # 管理 UI を 5578 で配信
# または
node scripts/serve-setup.mjs         # setup.html プレビューを 5582 で配信
```

ブラウザで `http://localhost:5578/` を開く (port は serve スクリプトの既定)。
