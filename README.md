...
ローカル開発・CLI でのセットアップ/デプロイ手順は **[docs/SETUP-cloudflare.md](docs/SETUP-cloudflare.md)** を参照。

- `npm run deploy` = `npm run db:migrate:remote && wrangler deploy`（本番 D1 へマイグレーションを適用してからデプロイ）。
- マイグレーションはバインディング名 `DB` 指定（`wrangler d1 migrations apply DB --remote`）。配布先の D1 が別名で生成されても同じスクリプトで動くよう統一。
